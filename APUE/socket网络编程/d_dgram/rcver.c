...
int main(){
    int sd; // 文件描述符
    struct sockadd_in laddr; // 本地地址和端口
    struct sockadd_in raddr; // 网络地址和端口
    struct msg_st *rbufp;
    int size = sizeof(struct msg_st) + NAMESIZE-1;
    rbufp = malloc(size);
    ...
    // 将地址和端口绑定到文件描述符中
    if(bind(sd, (void *)laddr, sizeof(laddr))) // strcut类型不一致需要强制转换
    {
        perror("bind()");
        exit(1);
    }
